...
- [x] Revise shuffle constraints, if an audio file is playing it and a user press shuffle for the queue it doesn't change that it is playing or the onces that have played. If no audio is playing and I press shuffle for the queue then the whole queue is shuffled and the selected audio file is set to the new top of list. — refined after initial feedback: "playback protected" now means playback has actually *started* (a hasPlaybackStarted flag set by the audio element's native "play" event), not merely that a track is primed/selected. Building a queue primes index 0 without playing it, so shuffling a freshly-built, never-played queue now reshuffles everything including index 0 — it no longer leaves the same track stuck at the front every time. Once playback genuinely starts, shuffle goes back to only reordering the unplayed tail. shuffleQueue() in audiobox_player.ts, unit-tested for both cases.
